...
NaraNote can check GitHub Releases for updates in the background. Updates are downloaded over HTTPS, verified against GitHub's SHA-256 asset digest when available, installed by a temporary updater after notes are saved, and then relaunched. The updater and backup are removed after a successful update; normal installation remains a single `NaraNote.exe`.
